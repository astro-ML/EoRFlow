...
output_dir = '/remote/gpu01a/pietschke/EoRFlow/output/SKA_flow/flow_6_256_weights_val'
...
# Here, skip_boundary_labels is True and we drop 90% of boundary samples,
# and we compute weights.
train_dataset = PowerSpectrumDataset(data_train, compute_weights=True)
...
def train_and_validate(model, train_loader, val_loader, optimizer, scheduler, num_epochs=50,
    ...
    for epoch in range(num_epochs):
        ...
        for ps_batch, target_batch, weight_batch in train_loader:
            ps_batch, target_batch = ps_batch.to(device), target_batch.to(device)
            weight_batch = weight_batch.to(device)
            optimizer.zero_grad()
            loss = weighted_flow_loss(model.flow, target_batch, ps_batch, model_params['flow']['n_dim'], weight_batch)
            loss.backward()
        ...
        epoch_val_loss = 0.0
        with torch.no_grad():
            for ps_batch, target_batch, weight_batch in val_loader:
                ps_batch, target_batch = ps_batch.to(device), target_batch.to(device)
                weight_batch = weight_batch.to(device)
                val_loss = weighted_flow_loss(model.flow, target_batch, ps_batch, model_params['flow']['n_dim'],
                                              weight_batch)
