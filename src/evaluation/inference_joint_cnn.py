...
class InferenceModel:
    ...
    Args:
        ...
        cnn_model (nn.Module): The pre-trained CNN model.
        flow_model (nn.Module): The flow model (normalizing flow).
        data_loader (Callable): Data loading function for 2DPS.
        device (str, optional): Device for computation ('cpu' or 'cuda'). Defaults to 'cpu'.
    """

    def __init__(self, params: dict, cnn_model: nn.Module, flow_model: nn.Module, data_dir: str, device: str = 'cpu') -> None:
        ...
        self.dataset = PowerSpectrumDataset_CNN(data_dir)
    ...
    def find_cnn_output(self, save_name: str = 'cnn_output_xH.npz') -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            self.cnn_pred = np.zeros((num_samples, 10))  # Adjust based on your CNN's output size + redshift
            self.label = np.zeros((num_samples, 3))
            ...
            for i, (ps_data, true_label) in enumerate(self.data_loader):
                ...
                pred = self.cnn_model(ps_data)
                # Repeat redshift values to match the batch size
                redshift_batch = redshifts.repeat(pred.size(0), 1)  # shape: [batch_size, 3]
                # Concatenate redshift information
                pred = torch.cat((pred, redshift_batch), dim=1)
    ...
    def calc_statistics(self, output_name: str = 'inference_statistics_xH.npz', sample_size: int = 1000) -> None:
        ...
        mean, lower, upper = np.zeros_like(self.cnn_pred), np.zeros_like(self.cnn_pred), np.zeros_like(self.cnn_pred)

        for i in range(self.cnn_pred.shape[0]):
            z = torch.randn((sample_size, 3)).to(self.device)
            samples, _ = self.flow_model(z, c=[torch.Tensor(self.cnn_pred[i]).repeat((sample_size, 1)).to(self.device)], rev=True)
    ...
    def plot_corner(self, index: int, sample_size: int = 1000) -> None:
        ...
        z = torch.randn((sample_size, 3)).to(self.device)
        samples, _ = self.flow_model(z, c=[torch.Tensor(self.cnn_pred[index]).repeat((sample_size, 1)).to(self.device)], rev=True)
        samples = samples.detach().cpu().numpy()
        mc_samples = MCSamples(samples=samples, names=['xH1', 'xH2', 'xH3'])
...
model_dir = '/remote/gpu01a/pietschke/EoRFlow/output/CNN_Flow_redshift'
...
cnn_model = CNN2D()
cnn_model.load_state_dict(torch.load(model_dir + '/cnn_model.pth'))
cnn_model.eval()
...
model_params = {
    'flow': {
        ...
        'n_blocks': 6,
        'n_nodes': 256,
        'cond_dims': 10,  # Adjust based on your CNN's output size
        'dropout': 0.0,
        'load': False,
...
flow_model.flow.load_state_dict(torch.load(model_dir + '/flow_model.pth'))
